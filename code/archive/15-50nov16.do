...
* Last Updated: 16 November 2025 [FINAL - i.sex version]
****************************************************
...
esttab desc_single desc_full using "$TABLES/table1_descriptives.tex", ///
    ...
    addnotes("Standard deviations in parentheses. Weighted by sample weights." ///
             ...
             "Female: Indicator for female (male=0, female=1). Sample: Ages 11, 13, 15; waves 2006, 2010, 2014; Belgium (Flemish).")
...
local balvars female agecat wave welloff

* Store joint F-test result first (includes wave since main spec has wave FE)
qui reg same_sex i.female i.agecat i.wave i.welloff [pw=weight] ///
    if sample_main_single, vce(cluster school)
testparm i.female i.agecat i.wave i.welloff
...
esttab using "$TABLES/table2_balance.tex", ///
    ...
    addnotes("Standard deviations in parentheses. Single-sibling sample only. Weighted means." ///
             "Female: Indicator for female (male=0, female=1). Joint F-test that all covariates jointly predict same\_sex: F(`joint_df',`joint_dfr') = `joint_f', p = `joint_p'." ///
             "Standard errors clustered at school level. Null hypothesis of no selection on observables cannot be rejected.")
...
* By affluence (linear interaction for parsimony)
* Note: Change to c.same_sex##i.welloff for fully flexible categorical interaction (as in earlier specs)
...
esttab h1 h2 h3 using "$TABLES/table5_heterogeneity.tex", ///
    ...
    addnotes("Ordered probit with interactions. All models control for child sex, age, wave, affluence." ///
             "Reference: Male (col 1), Age 11 (col 2). Col 3 uses linear interaction in affluence for parsimony." ///
             "Standard errors in parentheses, clustered at school level." ///
             "*** p\$<\$0.01, ** p\$<\$0.05, * p\$<\$0.10")
...
eststo ca1: qui oprobit acachieve_rev c.boy_both_sib##i.agecat ///
    ...
    i.sex i.wave i.welloff [pw=weight] ///
    if sample_multi_only, vce(cluster school)
...
esttab r1 r2 using "$TABLES/appendix_a2_2014_full.tex", ///
    ...
    stats(N, fmt(%9.0fc) labels("Observations")) ///
    mtitles("O.Probit" "OLS") ///
    title("Appendix A2: 2014 Full Sample Robustness Check\label{tab:2014full}") ///
...
di as result _n "=== TABLE GENERATION COMPLETE ===" _n
di as txt "Tables saved to: $TABLES"
di as txt ""
di as txt "Main Tables (for paper body):"
di as txt "  Table 1: Descriptive statistics"
di as txt "  Table 2: Covariate balance"
di as txt "  Table 3: Main results (4 specifications)"
di as txt "  Table 4: Marginal effects by outcome level"
di as txt "  Table 5: Heterogeneity analysis"
di as txt "  Table 6: Composition effects (multi-sibling)"
di as txt "  Table 7: 2014 wave analysis"
di as txt ""
di as txt "Appendix Tables (supplementary):"
di as txt "  A1: Composition by age"
di as txt "  A2: 2014 full sample robustness"
