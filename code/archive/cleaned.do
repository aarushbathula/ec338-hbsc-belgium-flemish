****************************************************
* Last Updated: 15 November 23:00
...
* 10. Descriptive statistics (log only, single-sibling sample)
*---------------------------------------------------
di as result _n "=== DESCRIPTIVE STATISTICS: SINGLE-SIBLING SAMPLE ===" _n

count if sample_main_single
di as txt "Sample: N = " r(N) " (single-sibling, age 11/13/15, non-missing key vars)" _n

di as text "CONTINUOUS VARIABLES"
summarize age acachieve_rev welloff if sample_main_single

di as text _n "CATEGORICAL VARIABLES (%)"
tab sex           if sample_main_single, nolabel
tab agecat        if sample_main_single, nolabel
tab same_sex      if sample_main_single, nolabel
tab wave          if sample_main_single, nolabel
tab acachieve_rev if sample_main_single, nolabel
tab welloff       if sample_main_single, nolabel
...
gen immigrant_origin = .    // default missing


replace immigrant_origin = 1 if inrange(m134, 2, 6) | inrange(m135, 2, 6)

replace immigrant_origin = 0 if ///
    ( (m134 == 1 | missing(m134)) & (m135 == 1 | missing(m135)) ) ///
    & !(missing(m134) & missing(m135))
...
*   - Single-sibling sample (main causal)
*   - Full sibling sample (>=1 sibling, robustness)
*===============================================================
...
* 14A. MAIN CAUSAL MODEL (SINGLE-SIBLING ONLY)
* Cleanest identification
...
* 14A.2 HETEROGENEITY: TEACHER TRUST (SINGLE-SIBLING ONLY)
...
* 14B. ROBUSTNESS (ALL SIBLINGS, siblingsum ≥1)
...
* 14B.2 Teacher Trust Heterogeneity (ALL siblings)
